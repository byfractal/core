...
@router.get("/cards", response_model=AnalysisResponse)
...
@router.get("/cards/{card_id}", response_model=AnalysisCard)
async def get_analysis_card_by_id(
    card_id: str = Path(..., description="ID of the analysis card"),
    service: AnalysisService = Depends(get_analysis_service)
):
    """
...
@router.post("/cards/{card_id}/mark-viewed")
...
@router.post("/apply-fix", response_model=Dict[str, Any])
async def apply_fix(
...
@router.post("/apply-all-fixes", response_model=Dict[str, Any])
...
async def get_import_status(
    import_id: str = Path(..., description="ID of the import process"),
    service: AnalysisService = Depends(get_analysis_service)
):
    """
...
@router.get("/available-pages/{import_id}", response_model=Dict[str, Any])
...
@router.post("/select-pages", response_model=Dict[str, Any])
